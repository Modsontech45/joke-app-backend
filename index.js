...
app.get("/random", (req,res)=>{
  let randomJokes = Math.random() * 100
  randomJokes = Math.floor(randomJokes);
  res.send(jokes[randomJokes])
})

//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  let jokeId = parseInt(req.params.id); // Convert string to number

  for (let joke of jokes) {  // Use "of" to loop through objects
    if (joke.id === jokeId) {
      return res.json(joke); // Return response immediately if found
    }
  }

  res.status(404).send("Joke not found"); // Send response after the loop
});
